...
#include "compatible/components/image-animator/image_animator_element.h"

#include "core/event/ace_event_helper.h"

namespace OHOS::Ace {
const char START_COMPATIBLE[] = "start";
const char PAUSE_COMPATIBLE[] = "pause";
const char STOP_COMPATIBLE[] = "stop";
const char RESUME_COMPATIBLE[] = "resume";
const char CANCEL_COMPATIBLE[] = "cancel";
...
void ImageAnimatorElement::PerformBuild()
{
    ...
    if (context_.Upgrade() && context_.Upgrade()->GetIsDeclarative()) {
        if (status_ == Animator::Status::IDLE) {
            CallAnimatorMethod(CANCEL_COMPATIBLE);
        } else if (status_ == Animator::Status::PAUSED) {
            CallAnimatorMethod(PAUSE_COMPATIBLE);
        } else if (status_ == Animator::Status::STOPPED) {
            CallAnimatorMethod(STOP_COMPATIBLE);
        } else {
            CallAnimatorMethod(START_COMPATIBLE);
        }
        return;
    }
...
void ImageAnimatorElement::CallAnimatorMethod(const std::string& method)
{
    ...
    if (method == START_COMPATIBLE) {
        isReverse_ ? animator_->Backward() : animator_->Forward();
    } else if (method == PAUSE_COMPATIBLE) {
        animator_->Pause();
    } else if (method == STOP_COMPATIBLE) {
        animator_->Finish();
    } else if (method == RESUME_COMPATIBLE) {
        animator_->Resume();
    } else if (method == CANCEL_COMPATIBLE) {
        animator_->Cancel();
    } else {
