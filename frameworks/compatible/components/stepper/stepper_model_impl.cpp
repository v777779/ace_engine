...
RefPtr<StepperTheme> GetTheme()
{
    ...
    return node ? themeManager->GetTheme<StepperTheme>(node->GetThemeScopeId())
        : themeManager->GetTheme<StepperTheme>();
}
...
void StepperModelImpl::Create(uint32_t index)
{
    ...
    RefPtr<StepperTheme> theme = GetTheme();
    if (theme) {
