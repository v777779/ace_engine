...
#include "compatible/components/grid_row/render_grid_row.h"

#include "compatible/components/grid_container/dom_grid_container_utils.h"
#include "compatible/components/grid_row/grid_row_component.h"
...
int32_t RenderGridRow::GetTotalCol() const
{
    ...
    auto totalColumn = DomGridContainerUtils::ProcessColumn(currentSizeType_, component->GetTotalCol());
    return totalColumn;
}
...
std::pair<double, double> RenderGridRow::GetGutter() const
{
    ...
    auto gutter = DomGridContainerUtils::ProcessGutter(currentSizeType_, component->GetGutter());
...
void RenderGridRow::PerformLayout()
{
    ...
    auto sizeType = DomGridContainerUtils::ProcessGridSizeType(component->GetBreakPoints(), maxSize, context);
    ...
    auto gutter = DomGridContainerUtils::ProcessGutter(sizeType, component->GetGutter());
    auto gutterInDouble = std::make_pair<double, double>(NormalizeToPx(gutter.first), NormalizeToPx(gutter.second));
    int32_t columnNum = DomGridContainerUtils::ProcessColumn(sizeType, component->GetTotalCol());
    double columnUnitWidth = DomGridContainerUtils::ProcessColumnWidth(gutterInDouble, columnNum, maxSize.Width());
