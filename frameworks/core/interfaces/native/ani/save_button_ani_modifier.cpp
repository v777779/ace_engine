    ...
    ani_env* env, const GestureEvent& info, ani_object& errAni, ani_enum_item& resultAni)
{
    ...
    if (secEventValue != nullptr) {
        ...
        env->FindEnum("arkui.component.saveButton.SaveButtonOnClickResult", &onClickResult);
...
class SaveButtonCallbackAni {
public:
    SaveButtonCallbackAni(ani_env* env, ani_ref func)
    {
        CHECK_NULL_VOID(env);
        env->GetVM(&vm_);
        env->GlobalReference_Create(func, &func_);
    }
    ~SaveButtonCallbackAni()
    {
        CHECK_NULL_VOID(vm_);
    ...
    void Call(ani_env* env, ani_size argc, ani_ref* argv, ani_ref* result)
    {
        ...
        if (status != ANI_OK) {
            SC_LOG_ERROR("SaveButtonCallbackAni call failed, status: %{public}d", status);
        }
    }
    ...
    ani_env* env, ani_object onClickCallback, std::function<void(const GestureEvent& info)>& callback)
{
    ...
    auto callbackAni = std::make_shared<SaveButtonCallbackAni>(env, fnObjGlobalRef);
...
{
    static const ArkUIAniSaveButtonModifier impl = {
        .setSaveButtonOnClickCallback = OHOS::Ace::NG::SetSaveButtonOnClickCallback,
    };
    return &impl;
}
} // namespace OHOS::Ace::NG