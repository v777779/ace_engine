...
{
#ifdef ACE_UNITTEST
    return GetCalendarPickerStaticModifier();
#else
    static const GENERATED_ArkUICalendarPickerModifier* cachedModifier = nullptr;
    if (!cachedModifier) {
        auto* module = DynamicModuleHelper::GetInstance().GetDynamicModule("CalendarPicker");
        CHECK_NULL_RETURN(module, nullptr);
        cachedModifier = reinterpret_cast<const GENERATED_ArkUICalendarPickerModifier*>(module->GetStaticModifier());
    }
