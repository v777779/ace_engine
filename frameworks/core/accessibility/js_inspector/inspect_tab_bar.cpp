...
void InspectTabBar::PackAttrAndStyle()
{
    // add for the attrs
    attrs_.insert(std::make_pair("mode", "scrollable"));
    ...
    if (deviceType == DeviceType::PHONE) {
        styles_.insert(std::make_pair("height", "100px"));
    } else if (deviceType == DeviceType::TV) {
        styles_.insert(std::make_pair("height", "60px"));
    }
}
} // namespace OHOS::Ace::Framework
