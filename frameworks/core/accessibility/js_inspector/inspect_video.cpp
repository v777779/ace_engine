...
void InspectVideo::PackAttrAndStyle()
{
    // add for the attrs
    attrs_.insert(std::make_pair("muted", "false"));
    attrs_.insert(std::make_pair("autoplay", "false"));
    attrs_.insert(std::make_pair("controls", "true"));
    attrs_.insert(std::make_pair("disabled", "false"));
    attrs_.insert(std::make_pair("focusable", "true"));

    // add for the styles
    styles_.insert(std::make_pair("object-fit", "contain"));
}
} // namespace OHOS::Ace::Framework
