...
bool CheckBoxAccessibilityProperty::IsCheckable() const
{
    return true;
}

bool CheckBoxAccessibilityProperty::IsChecked() const
{
    auto frameNode = host_.Upgrade();
    CHECK_NULL_RETURN(frameNode, false);
    auto paintProperty = frameNode->GetPaintProperty<CheckBoxPaintProperty>();
    CHECK_NULL_RETURN(paintProperty, false);
    return paintProperty->GetCheckBoxSelect().value_or(false);
}

std::string CheckBoxAccessibilityProperty::GetText() const
{
    auto frameNode = host_.Upgrade();
