...
#ifndef FOUNDATION_ACE_FRAMEWORKS_CORE_COMPONENTS_NG_PATTERN_WEB_WEB_CONTEXT_MENU_OVERLAY_H
#define FOUNDATION_ACE_FRAMEWORKS_CORE_COMPONENTS_NG_PATTERN_WEB_WEB_CONTEXT_MENU_OVERLAY_H
...
public:
    explicit WebContextMenuOverlay(const WeakPtr<TextBase>& textBase) : BaseTextSelectOverlay(textBase) {}
    ~WebContextMenuOverlay() = default;
    ...
    void OnHandleGlobalEvent(const PointF& touchPoint, const PointF& localPoint, const TouchEvent& event) override;
    // override SelectOverlayCallback end

    void SetEditMenuOption(SelectOverlayInfo& selectInfo);
    std::vector<NG::MenuItemParam> FilterSupportedMenuItems(const std::vector<NG::MenuItemParam>& menuItemList);
...
#endif // FOUNDATION_ACE_FRAMEWORKS_CORE_COMPONENTS_NG_PATTERN_WEB_WEB_CONTEXT_MENU_OVERLAY_H