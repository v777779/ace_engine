...
void FolderStackPattern::OnDetachFromFrameNodeMultiThread(FrameNode* node)
{
    // nothing, thread unsafe
}
void FolderStackPattern::OnAttachToMainTreeMultiThread()
{
    auto host = GetHost();
...
void FolderStackPattern::OnDetachFromMainTreeMultiThread()
{
    auto host = GetHost();
    CHECK_NULL_VOID(host);
    auto pipeline = host->GetContext();
