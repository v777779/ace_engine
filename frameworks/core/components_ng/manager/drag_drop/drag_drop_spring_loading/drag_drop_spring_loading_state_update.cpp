...
void DragDropSpringLoadingStateUpdate::OnEnter(std::string_view extraInfo)
{
    ...
    stateMachine->IncrementCurrentUpdateNotifyCount();
    callback(context);
    ...
    if (userConfig->updateNotifyCount <= stateMachine->GetCurrentUpdateNotifyCount()) {
