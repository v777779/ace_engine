...
def create_stamp_file(stamp_path):
    try:
        ...
        print(f"[NPM_OP ERROR] Failed to create stamp file: {e}")
        return False

def main():
    if len(sys.argv) != 3:
        print("[NPM_OP INFO] Usage: npm_install.py <project_path> <stamp_path>")
        sys.exit(1)
    ...
    print("[NPM_OP INFO] Node modules not found, running npm install...")
    try:
        subprocess.check_call(["npm", "install"], cwd=project_path)
    except subprocess.CalledProcessError as e:
        print(f"[NPM_OP ERROR] npm install failed with exit code {e.returncode}. Retry with registry...")
        print(e.stderr)
        return e.returncode

    if create_stamp_file(stamp_path):
        print("[NPM_OP INFO] npm install completed.")
        return 0
    else:
        return 1
