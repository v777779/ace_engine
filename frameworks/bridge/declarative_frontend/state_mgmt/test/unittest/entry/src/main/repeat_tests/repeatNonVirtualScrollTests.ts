...
declare class __RepeatImpl<T> {
    public arr_;
    public itemGenFuncs_;
    public keyGenFunction_?;
    public ttypeGenFunc_;
    public mkRepeatItem_;
    public onMoveHandler_?;
    public itemDragEventHandler?;
    public key2Item_;
    constructor();
    render(config: __RepeatConfig<T>, isInitialRender: boolean): void;
    getKey2Item(): Map<string, __RepeatItemInfo<T>>;
    public genKeys;
    public initialRender;
    public reRender;
    public afterAddChild;
    public initialRenderItem;
}

export class RepeatNonVirtualScrollTest1 implements ITestFile {
  ...
  /* Test creation of __RepeatImpl
  * Calls:
   __RepeatImpl.constructor
  */
  public RepeatCreate(): void {
    let repeat = new __RepeatImpl();
    repeat.arr_ = [ 0, 1, 2, 3, 4 ];
    repeat.itemGenFuncs_ = { ['']: () => {} };
    repeat.ttypeGenFunc_ = () => { return ''; };
    repeat.keyGenFunction_ = (item) => { return JSON.stringify(item); };
    repeat.mkRepeatItem_ = (item, index?) => new __RepeatItemV2(item, index);
    repeat.onMoveHandler_ = undefined;
    repeat.itemDragEventHandler = undefined;
    eq(repeat.arr_.length, 5, 'RepeatNonVirtualScrollTest1_RepeatCreate_01');
    eq(repeat.key2Item_.size, 0, 'RepeatNonVirtualScrollTest1_RepeatCreate_02');
  }
}