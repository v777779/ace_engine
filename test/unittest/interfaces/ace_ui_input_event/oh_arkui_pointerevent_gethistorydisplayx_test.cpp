...
HWTEST_F(UIInputEventTest, OH_ArkUI_PointerEvent_GetHistoryDisplayX_001, TestSize.Level0)
{
    auto result = OH_ArkUI_PointerEvent_GetHistoryDisplayX(nullptr, 0, 0);
    EXPECT_FLOAT_EQ(result, HISTORY_X_ERROR);
}

HWTEST_F(UIInputEventTest, OH_ArkUI_PointerEvent_GetHistoryDisplayX_002, TestSize.Level0)
{
    ...
    for (auto testCase : testCases) {
        ...
        auto result = OH_ArkUI_PointerEvent_GetHistoryDisplayX(&input.event, input.pointerIndex, input.historyIndex);
        EXPECT_FLOAT_EQ(result, expect) << "index = " << count << ",result = " << result << ",expect = " << expect;
        count++;
    }
}

HWTEST_F(UIInputEventTest, OH_ArkUI_PointerEvent_GetHistoryDisplayX_003, TestSize.Level0)
{
    ArkUIHistoryTouchEvent events[EVENT_SIZE];
    ArkUIHistoryTouchEvent emptyEvents[EVENT_SIZE];
    ArkUITouchPoint pointes[POINT_SIZE];
    pointes[0].screenX = ARKUI_X;
    pointes[1].screenX = ARKUI_X;
