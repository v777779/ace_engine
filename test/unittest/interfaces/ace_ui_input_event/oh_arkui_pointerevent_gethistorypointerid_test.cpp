...
namespace OHOS::Ace {
namespace {
constexpr float HISTORY_ID_ONE = 1;
constexpr float HISTORY_ID_TWO = 2;
...
HWTEST_F(UIInputEventTest, OH_ArkUI_PointerEvent_GetHistoryPointerId_001, TestSize.Level0)
{
    auto result = OH_ArkUI_PointerEvent_GetHistoryPointerId(nullptr, 0, 0);
    EXPECT_EQ(result, 0);
}

HWTEST_F(UIInputEventTest, OH_ArkUI_PointerEvent_GetHistoryPointerId_002, TestSize.Level0)
{
    ...
    std::vector<std::pair<HistoryTestInputType, int32_t>> testCases = {
    ...
    for (auto testCase : testCases) {
        ...
        auto result = OH_ArkUI_PointerEvent_GetHistoryPointerId(&input.event, input.pointerIndex, input.historyIndex);
        EXPECT_EQ(result, expect) << "index = " << count << ",result = " << result << ",expect = " << expect;
        count++;
    }
}

HWTEST_F(UIInputEventTest, OH_ArkUI_PointerEvent_GetHistoryPointerId_003, TestSize.Level0)
{
    ArkUIHistoryTouchEvent events[EVENT_SIZE];
    ArkUIHistoryTouchEvent emptyEvents[EVENT_SIZE];
    ArkUITouchPoint pointes[POINT_SIZE];
    pointes[0].id = HISTORY_ID_ONE;
    pointes[1].id = HISTORY_ID_TWO;
    ...
    std::vector<std::pair<HistoryTestInputType, float>> testCases = {
        ...
        { HistoryTestInputType(
              ArkUI_UIInputEvent { ARKUI_UIINPUTEVENT_TYPE_UNKNOWN, C_TOUCH_EVENT_ID, &nodeEvent.touchEvent }, 1, 1,
              emptyEvents, EVENT_SIZE),
            0 }, // test when (touchEvent->historyEvents[historyIndex].touchPointes) = nullptr
        { HistoryTestInputType(
              ArkUI_UIInputEvent { ARKUI_UIINPUTEVENT_TYPE_UNKNOWN, C_TOUCH_EVENT_ID, &nodeEvent.touchEvent }, 1, 1,
              events, EVENT_SIZE),
            HISTORY_ID_TWO }, // test when valid input with pointerIndex valid
        { HistoryTestInputType(
              ArkUI_UIInputEvent { ARKUI_UIINPUTEVENT_TYPE_UNKNOWN, C_TOUCH_EVENT_ID, &nodeEvent.touchEvent }, 1, 0,
              emptyEvents, EVENT_SIZE),
            0 }, // test when pointerIndex = Lower boundary,
                 // (touchEvent->historyEvents[historyIndex].touchPointes) = nullptr
        { HistoryTestInputType(
              ArkUI_UIInputEvent { ARKUI_UIINPUTEVENT_TYPE_UNKNOWN, C_TOUCH_EVENT_ID, &nodeEvent.touchEvent }, 1, 0,
              events, EVENT_SIZE),
            HISTORY_ID_TWO }, // test when valid input with pointerIndex = Lower boundary

        { HistoryTestInputType(
              ArkUI_UIInputEvent { ARKUI_UIINPUTEVENT_TYPE_UNKNOWN, C_TOUCH_EVENT_ID, &nodeEvent.touchEvent }, 0, 0),
            0 }, // test when historyIndex = Lower boundary, touchEvent->historyEvents = nullptr
        { HistoryTestInputType(
              ArkUI_UIInputEvent { ARKUI_UIINPUTEVENT_TYPE_UNKNOWN, C_TOUCH_EVENT_ID, &nodeEvent.touchEvent }, 0, -1,
              emptyEvents, EVENT_SIZE),
            0 }, // test when historyIndex = Lower boundary, pointerIndex < 0
        { HistoryTestInputType(
              ArkUI_UIInputEvent { ARKUI_UIINPUTEVENT_TYPE_UNKNOWN, C_TOUCH_EVENT_ID, &nodeEvent.touchEvent }, 0, 10,
              emptyEvents, EVENT_SIZE),
            0 }, // test when historyIndex = Lower boundary, pointerIndex >
        ...
        { HistoryTestInputType(
              ArkUI_UIInputEvent { ARKUI_UIINPUTEVENT_TYPE_UNKNOWN, C_TOUCH_EVENT_ID, &nodeEvent.touchEvent }, 0, 1,
              emptyEvents, EVENT_SIZE),
            0 }, // test when historyIndex = Lower boundary,
                 // (touchEvent->historyEvents[historyIndex].touchPointes) = nullptr
        { HistoryTestInputType(
              ArkUI_UIInputEvent { ARKUI_UIINPUTEVENT_TYPE_UNKNOWN, C_TOUCH_EVENT_ID, &nodeEvent.touchEvent }, 0, 1,
              events, EVENT_SIZE),
            HISTORY_ID_ONE }, // test when historyIndex = Lower boundary, input valid
        { HistoryTestInputType(
              ArkUI_UIInputEvent { ARKUI_UIINPUTEVENT_TYPE_UNKNOWN, C_TOUCH_EVENT_ID, &nodeEvent.touchEvent }, 0, 0,
              emptyEvents, EVENT_SIZE),
            0 }, // test when historyIndex = Lower boundary, pointerIndex = Lower boundary,
                 // (touchEvent->historyEvents[historyIndex].touchPointes) = nullptr
        { HistoryTestInputType(
              ArkUI_UIInputEvent { ARKUI_UIINPUTEVENT_TYPE_UNKNOWN, C_TOUCH_EVENT_ID, &nodeEvent.touchEvent }, 0, 0,
              events, EVENT_SIZE),
            HISTORY_ID_ONE }, // test when historyIndex = Lower boundary, pointerIndex = Lower boundary, input valid
    };
