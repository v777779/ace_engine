...
constexpr float ARKUI_WIDTH = 1234.0f;
...
HWTEST_F(UIInputEventTest, OH_ArkUI_PointerEvent_GetHistoryTouchAreaWidth_001, TestSize.Level0)
{
    auto result = OH_ArkUI_PointerEvent_GetHistoryTouchAreaWidth(nullptr, 0, 0);
    EXPECT_FLOAT_EQ(result, HISTORY_X_ERROR);
}

HWTEST_F(UIInputEventTest, OH_ArkUI_PointerEvent_GetHistoryTouchAreaWidth_002, TestSize.Level0)
{
    ...
    for (auto testCase : testCases) {
        ...
        auto result = OH_ArkUI_PointerEvent_GetHistoryTouchAreaWidth(&input.event, input.pointerIndex, input.historyIndex);
        EXPECT_FLOAT_EQ(result, expect) << "index = " << count << ",result = " << result << ",expect = " << expect;
        count++;
    }
}

HWTEST_F(UIInputEventTest, OH_ArkUI_PointerEvent_GetHistoryTouchAreaWidth_003, TestSize.Level0)
{
    ArkUIHistoryTouchEvent events[EVENT_SIZE];
    ArkUIHistoryTouchEvent emptyEvents[EVENT_SIZE];
    ArkUITouchPoint pointes[POINT_SIZE];
    pointes[0].contactAreaWidth = ARKUI_WIDTH;
    pointes[1].contactAreaWidth = ARKUI_WIDTH;
    ...
    std::vector<std::pair<HistoryTestInputType, float>> testCases = {
        ...
        { HistoryTestInputType(
              ArkUI_UIInputEvent { ARKUI_UIINPUTEVENT_TYPE_UNKNOWN, C_TOUCH_EVENT_ID, &nodeEvent.touchEvent }, 1, 1,
              events, EVENT_SIZE),
            ARKUI_WIDTH }, // test when valid input with pointerIndex valid
        { HistoryTestInputType(
              ArkUI_UIInputEvent { ARKUI_UIINPUTEVENT_TYPE_UNKNOWN, C_TOUCH_EVENT_ID, &nodeEvent.touchEvent }, 1, 0,
              emptyEvents, EVENT_SIZE),
        ...
        { HistoryTestInputType(
              ArkUI_UIInputEvent { ARKUI_UIINPUTEVENT_TYPE_UNKNOWN, C_TOUCH_EVENT_ID, &nodeEvent.touchEvent }, 1, 0,
              events, EVENT_SIZE),
            ARKUI_WIDTH }, // test when valid input with pointerIndex = Lower boundary
        ...
        { HistoryTestInputType(
              ArkUI_UIInputEvent { ARKUI_UIINPUTEVENT_TYPE_UNKNOWN, C_TOUCH_EVENT_ID, &nodeEvent.touchEvent }, 0, 1,
              events, EVENT_SIZE),
            ARKUI_WIDTH }, // test when historyIndex = Lower boundary, input valid
        { HistoryTestInputType(
              ArkUI_UIInputEvent { ARKUI_UIINPUTEVENT_TYPE_UNKNOWN, C_TOUCH_EVENT_ID, &nodeEvent.touchEvent }, 0, 0,
              emptyEvents, EVENT_SIZE),
        ...
        { HistoryTestInputType(
              ArkUI_UIInputEvent { ARKUI_UIINPUTEVENT_TYPE_UNKNOWN, C_TOUCH_EVENT_ID, &nodeEvent.touchEvent }, 0, 0,
              events, EVENT_SIZE),
            ARKUI_WIDTH }, // test when historyIndex = Lower boundary, pointerIndex = Lower boundary, input valid
    };
