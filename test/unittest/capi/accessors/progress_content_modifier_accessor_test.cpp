...
class ProgressContentModifierHelperAccessor : public StaticAccessorTest<GENERATED_ArkUIContentModifierHelperAccessor,
                                &GENERATED_ArkUIAccessors::getContentModifierHelperAccessor> {
public:
    void FireBuilder(ProgressPattern* pattern)
    {
        pattern->FireBuilder();
    }
};
...
static constexpr float TEST_DEFAULT_VALUE = 0;
...
HWTEST_F(ProgressContentModifierHelperAccessor, contentModifierProgressTest, TestSize.Level1)
{
    ...
    RefPtr<FrameNode> testNode = FrameNode::GetOrCreateFrameNode(
            V2::PROGRESS_ETS_TAG, TEST_NODE_ID, []() { return AceType::MakeRefPtr<ProgressPattern>(); });
    ASSERT_NE(testNode, nullptr);
    auto pattern = testNode->GetPattern<ProgressPattern>();
    ...
    auto modifierCallback = [](const Ark_Int32 resourceId, const Ark_NativePointer parentNode,
        const Ark_ProgressConfiguration config, const Callback_Pointer_Void continuation) {
            ...
            checkEvent = {
                ...
                .value = Converter::OptConvert<float>(config.value),
                .total = Converter::OptConvert<float>(config.total),
            };
    };

    EXPECT_CALL(*MockContainer::Current(), GetFrontend()).WillRepeatedly(Return(nullptr));

    auto builder = Converter::ArkValue<ProgressModifierBuilder>(modifierCallback, TEST_BUILDER_ID);
    Ark_NativePointer nodePtr = reinterpret_cast<Ark_NativePointer>(testNode.GetRawPtr());
    accessor_->contentModifierProgress(nodePtr, &obj, &builder);
    ...
    ASSERT_TRUE(checkEvent->value.has_value()) << "value is not set";
