 ...
 * Run via: npx ts-node ignore_layout_simulator.ts [path/to/data.json]
 */
...
interface TestData {
  tests: TestEntry[];
  knownCorrect: Record<string, string[]>;
}
...
class SimNode {
  ...
  private _delaySelfLayoutForIgnore = false;
  hasPreMeasured = false;
  ignoreLayoutProcess = false;
  delayMeasureChildren: SimNode[] = [];
  delayLayoutChildren: SimNode[] = [];

  constructor(config: NodeConfig, parent: SimNode | null = null) {
  ...
  getDelaySelfLayoutForIgnore(): boolean {
...
type Bundle = { first: SimNode[]; second: SimNode };
...
class Simulator {
  ...
  simulate(rootConfig: NodeConfig): string[] {
    this.events = [];
    this.bundleQueue = [];
  ...
  createLayoutTask(
    node: SimNode,
    layoutType: "NORMAL" | "MEASURE_FOR_IGNORE" | "LAYOUT_FOR_IGNORE"
  ) {
   ...
   * Leaf: no children
   */
  algorithmMeasure(node: SimNode) {
    this.record(`${node.name}:Measure`);

    if (node.type === "leaf") return;

    if (node.type === "flex") {
      const localBundle: SimNode[] = [];
...
function loadTestData(jsonPath: string): TestData {
