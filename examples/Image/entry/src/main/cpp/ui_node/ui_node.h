...
#ifndef CAPI_UI_NODE_H
#define CAPI_UI_NODE_H
...
namespace Self::UI {
class UINode : public BaseNode {
    public:
        UINode(ArkUI_NodeHandle handle, const std::string& tag) : BaseNode(handle, tag) {
            ...
            // 注册节点监听事件接收器。
            nativeModule_->addNodeEventReceiver(handle_, UINode::NodeEventReceiver);
        }
        ~UINode() override 
        {
            if (onClick_) {
            ...
            nativeModule_->unregisterNodeEvent(handle_, UINode::NodeEventReceiver);
        }
        ...
        void SetWidthLayoutPolicy(LayoutPolicy policy) 
        {
            ArkUI_NumberValue value[] = {{.i32 = static_cast<int>(policy)}};
            ArkUI_AttributeItem item = {value, 1};
            nativeModule_->setAttribute(handle_, NODE_WIDTH_LAYOUTPOLICY, &item);
        }
        
        void SetHeightLayoutPolicy(LayoutPolicy policy) 
        {
            ArkUI_NumberValue value[] = {{.i32 = static_cast<int>(policy)}};
            ArkUI_AttributeItem item = {value, 1};
    ...
    protected:
        // 组件树操作的实现类对接
        void OnAddChild(const std::shared_ptr<BaseNode> &child) override 
        {
            nativeModule_->addChild(handle_, child->GetHandle());
        }
        void OnRemoveChild(const std::shared_ptr<BaseNode> &child) override 
        {
            nativeModule_->removeChild(handle_, child->GetHandle());
        }
        void OnInsertChild(const std::shared_ptr<BaseNode> &child, int32_t index) override 
        {
        ...
        static void NodeEventReceiver(ArkUI_NodeEvent *event) 
        {
            ...
            auto *node = reinterpret_cast<UINode*>(
        ...
        {
            auto eventType = ON_ArkUI_NodeEvent_GetEventType(event);
            switch (eventType) {
                case NODE_ON_CLICK: {
                    if (onClick_) {
                        onClick_();
