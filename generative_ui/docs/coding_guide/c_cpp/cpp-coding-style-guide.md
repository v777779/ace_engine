...
When C and C++ programmings are used together and if extern "C" includes a header file, the original behind the header file may be hindered. For example, when the link specifications are modified incorrectly.
...
auto ea = ca;         // int, const and reference are neglected.
...
Alternatively, use `0` directly where null pointers are required. However, another problem occurs. The code is not clear, especially when `auto` is used for automatic deduction.
